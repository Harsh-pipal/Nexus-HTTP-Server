...
class NexusUtilities {
    ...
    static void loadMIMETypes(map<string, string> &mimeTypesMap) {
        FILE *file;
        file = fopen("Nexus Data/mime.types", "r");
        if(file == NULL)
            return;
        char *mimeType;
        char *extension;
        char line[200];         //expecting no line in apache mime file will exceed 199 characters
        int x;
        while(true) {
            fgets(line, 200, file);
            if(feof(file)) break;
            ...
            //Logic to remove \r \n from the end of the string (Starts)
            x = strlen(line) - 1;
            ...
            //Logic to remove \r \n from the end of the string (Ends)
            mimeType = &line[0];
            for(x = 0; line[x] != '\t'; x++);
            line[x] = '\0';
            x++;
...
class HttpErrorStatusUtility {
    ...
    static void sendNotFoundError (int clientSocketDescriptor, char *requestURI) {
        //optimization will be done here later
        char content[1000];
        char header[200];
        char response[1200];
    ...
    void createDataMap(char *str, map<string, string> &dataMap) {
        ...
        while(1) {
            ...
            *ptr2 = '\0';
            ...
            NexusUtilities::decode(ptr1, decoded);
...
enum __request_method__{__GET__, __POST__, __PUT__, __DELETE__, __CONNECT__, __HEAD__, __TRACE__, __OPTIONS__ };
...
class Nexus {
    ...
    Nexus() {
        //not yet decided
        NexusUtilities::loadMIMETypes(mimeTypes);
        if(mimeTypes.size() == 0) throw string("Nexus data has been tampered with...");
    }
    ~Nexus() {   
        //not yet decided
    }
    ...
    bool serverStaticResource(int clientSocketDescriptor, const char* requestURI) {
        ...
        while(bytesLeftToRead > 0) {
            ...
            if(feof(file)) break; //this wont happen in our case
    ...
    void listen( int portNumber, void (*callBack)(Error &)) {
        ...
        Error error("");
        callBack(error);
        struct sockaddr_in clientSocketInfomation;
        #ifdef linux
        socklen_t len= sizeof(clientSocketInfomation);
        #endif
        #ifdef _WIN32 
        int len = sizeof(clientSocketInfomation);
        #endif
        
        int clientSocketDescriptor;
        while(1) {
            clientSocketDescriptor = accept(serverSocketDescriptor, (struct sockaddr *)&clientSocketInfomation, &len);
            ...
            if(!(strcmp(method, "get") == 0 || strcmp(method, "put") == 0 || strcmp(method, "post") == 0 || strcmp(method, "delete") == 0 || strcmp(method, "trace") == 0 || strcmp(method, "options") == 0 || strcmp(method, "header") == 0)) {
                HttpErrorStatusUtility::sendBadRequestError(clientSocketDescriptor);
                close(clientSocketDescriptor);
                continue;
            }
...
//Bobby the user of Nexus server
int main() {
    ...
    try{
    Nexus nexus;
    nexus.setStaticResourcesFolder("static");
    nexus.get("/", [] (Request &request, Response &response){
        response<<R""""(
            <!DOCTYPE HTML>
            <html lang = 'en'>
            <head>
            <meta charset='utf-8'>
            <title>Nexus Student List</title>
            </head>
            <body>
            ...
            <thead>
            <tr>
            <th>Sr.No. </th><th>Roll Number </th><th>Name </th><th>Gender </th><th>Edit </th><th>Delete </th>
            </tr>
            </thead>
            </body>
        )"""";
    ...
    nexus.get("/addStudent", [] (Request &request, Response &response) {
        ...
        response<<R""""(
            ...
            <head>
            <meta charset = 'utf-8'>
            <title> Nexus - Student form </title>
            </head>
            <body>
    ...
    // nexus.get("/save_test1_data", [] (Request &request, Response &response){
    ...
    // nexus.post("/save_test2_data", [](Request & request, Response &response){
    //     const char *html = R""""(
    //         <!DOCTYPE HTML>
    //         <html lang = 'en'>
    //         <head>
    ...
    nexus.listen(8080, [](Error &error){
        if(error.hasError()) {
            cout<<error.getError();
            return;
        }
        cout<<"Nexus HTTP server is ready to accept request on port 8080" << endl;
    });
